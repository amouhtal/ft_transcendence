...
export class AuthService extends PassportStrategy(Strategy, '42') {
  ...
  async refresh(refreshStr: string): Promise<string | undefined> {
    ...
    const user = await this.usersRepository.findOneBy( {email : refreshToken.email});
  ...
  async Login(req, res,values : {ipAddress: string}) 
  {
    ...
    return  { refAcc: await this.newRefreshAndAccessToken(userDto.email, false, values), UserEmail : userDto.email };
  }
}
